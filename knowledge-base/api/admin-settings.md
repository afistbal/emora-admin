...
上下文整理边界以普通内部配置保存于 `settings` 表，管理后台在系统设置下方的 Key/Value 列表中查询和编辑，不单独展示配置卡片。

- **配置键**：`ai.context.memory_extraction_prompt`
- **值类型**：`string`
- **公开范围**：内部
- **状态**：启用
- **管理接口**：统一使用本页的配置列表、添加和更新接口。
- **运行行为**：配置缺失、停用或类型错误时使用服务端内置的严格默认指令；有效修改从下一次上下文记忆整理任务开始生效。
- **模型**：整理任务使用管理后台当前启用的文本模型，不固定使用 Mem0 容器中的模型配置。

以下专用接口仅为已上线版本的向后兼容入口；当前管理后台不再调用：

### 获取配置

- **请求**：`POST https://testapi.weshow.cc/api/admin/settings/context-memory`
- **说明**：返回长期记忆整理 AI 的独立边界指令。配置不存在或停用时返回服务端内置的安全默认指令。

响应 `d`：

```json
{
  "config_key": "ai.context.memory_extraction_prompt",
  "prompt": "上下文整理边界指令"
}
```
...
- **请求**：`POST https://testapi.weshow.cc/api/admin/settings/context-memory/save`
- **参数**：`prompt`，必填字符串，去除首尾空白后长度为 1–32000。
- **行为**：保存到现有 `settings` 表并递增版本；从下一次上下文记忆整理任务开始生效。
- **模型**：整理任务使用管理后台当前启用的文本模型，不再固定使用 mem0 容器的模型配置。
