...
export default function SettingsPage({ adminToken }) {
  ...
  const columns = [
    ...
    { title: "开关", dataIndex: "status", key: "status", width: 110, render: (status, item) => <Switch checked={Number(status) === 1} loading={savingId === item.id && savingField === "status"} disabled={savingId !== null} checkedChildren="启用" unCheckedChildren="停用" onChange={(checked) => toggleStatus(item, checked)} /> },
